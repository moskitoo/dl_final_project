...
def get_dataloader(sample_size, batch_size):
    ...
    mask_roo = '/zhome/70/5/14854/nobackup/deeplearningf24/forcebiology/data/masks'
    ...
    brightfield_train_datatset = BrightfieldMicroscopyDataset(train=True, transform=transform_train)
    brightfield_val_datatset = BrightfieldMicroscopyDataset(train=False, validation=True, transform=transform_val)
...
def train_model(model, train_loader, val_loader, optimiser, lr_scheduler, criterion, device, args, early_stopping, num_epochs=10):
    # Initialize W&B run
    wandb.init(
...
if __name__ == '__main__':
    ...
    train_loader, val_loader = get_dataloader(args.sample_size, args.batch_size)
